...
/* static */
void ConfigReader::Trim(string& inout_s)
{
...
{
	// Save a Config to os  
	for (ConfigReader::mapci p = cf.m_Contents.begin();
		p != cf.m_Contents.end();
		++p)
	{
...
bool ConfigReader::FileExist(std::string filename)
{
	bool exist = false;
	std::ifstream in(filename.c_str());
	if (in)
...
void ConfigReader::ReadFile(string filename, string delimiter,
	string comment)
{
