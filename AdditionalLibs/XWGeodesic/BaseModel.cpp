...
void CBaseModel::NormalizeModel()
{
	...
	{
		CPoint3D pt1 = Vert(Face(i)[0]);
		CPoint3D pt2 = Vert(Face(i)[1]);
		CPoint3D pt3 = Vert(Face(i)[2]);
		CPoint3D normal = VectorCross(pt1, pt2, pt3);
		double area = normal.Len() / 2;
		totArea += area;
	}
	double sqrtTotArea = sqrt(totArea);
	for (int i = 0; i < GetNumOfVerts(); i++)
	{
		m_Verts[i].x /= sqrtTotArea;
		m_Verts[i].y /= sqrtTotArea;
		m_Verts[i].z /= sqrtTotArea;
	}
	totArea = 0;
	for (int i = 0; i < GetNumOfFaces(); i++)
	{
	...
	assert(totArea >= 0.999&&totArea <= 1.001);
}
...
bool InsideBoundedArea(const map<int, int>& oldToNew, int v1, int v2, int v3)
{
	if (oldToNew.find(v1) != oldToNew.end()
		&& oldToNew.find(v2) != oldToNew.end()
		&& oldToNew.find(v3) != oldToNew.end())
		return true;
	return false;
}

void CBaseModel::SavePamametrizationObjFile(const map<int, pair<double, double>>& uvs, const string& filename) const
{
	map<int, int> oldToNew;
	int id(0);
	for (map<int, pair<double, double>>::const_iterator it = uvs.begin();
		it != uvs.end(); ++it)
	{
		oldToNew[it->first] = id++;
	}
	...
	for (map<int, pair<double, double>>::const_iterator it = uvs.begin();
		it != uvs.end(); ++it)
	{
		outFile << "v " << Vert(it->first).x << " " << Vert(it->first).y << " " << Vert(it->first).z << endl;
	}
	for (map<int, pair<double, double>>::const_iterator it = uvs.begin();
		it != uvs.end(); ++it)
	{
		outFile << "vt " << it->second.first << " " << it->second.second << endl;
	}
	for (int i = 0; i < (int)GetNumOfFaces(); ++i)
	{
		//if (m_UselessFaces.find(i) != m_UselessFaces.end())
		//	continue;
		if (!InsideBoundedArea(oldToNew, Face(i)[0], Face(i)[1], Face(i)[2]))
			continue;
		int v1 = oldToNew[Face(i)[0]];
		int v2 = oldToNew[Face(i)[1]];
		int v3 = oldToNew[Face(i)[2]];
		outFile << "f " << v1+ 1 << "/" << v1 + 1
			<< " " << v2 + 1 << "/" << v2 + 1
			<< " " << v3 + 1 << "/" << v3 + 1 << endl;
	}
...
vector<double> CBaseModel::GetScalarField(const char* filename)
{
