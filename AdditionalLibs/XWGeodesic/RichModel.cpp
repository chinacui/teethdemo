...
void CRichModel::RenewEdgeLength(int edgeIndex)
{
	int leftV = Edge(edgeIndex).indexOfLeftVert;
	int rightV = Edge(edgeIndex).indexOfRightVert;
	m_Edges[edgeIndex].length = (Vert(leftV) - Vert(rightV)).Len();
	
}

void CRichModel::RenewNeighboursAfterSplit(int newV, int oldEdge, const set<int>& newEdges)
{
	set<int> allVertices;
	allVertices.insert(Edge(oldEdge).indexOfLeftVert);
	allVertices.insert(Edge(oldEdge).indexOfRightVert);
	allVertices.insert(Edge(oldEdge).indexOfOppositeVert);
	allVertices.insert(Edge(Edge(oldEdge).indexOfReverseEdge).indexOfOppositeVert);

	set<int> allAffectedEdges;
	for (set<int>::const_iterator it = allVertices.begin(); it != allVertices.end(); ++it)
	{
		int degree = Neigh(*it).size();
		for (int i = 0; i < degree; ++i)
		{
			if (Neigh(*it)[i].first != oldEdge
				&& Neigh(*it)[i].first != Edge(oldEdge).indexOfReverseEdge)
			allAffectedEdges.insert(Neigh(*it)[i].first);
		}
		m_NeighsAndAngles[*it].clear();
	}
	
	for (set<int>::const_iterator it = newEdges.begin(); it != newEdges.end(); ++it)
	{
		allAffectedEdges.insert(*it);
	}
	assert(m_NeighsAndAngles.size() == newV);
	allVertices.insert(newV);
	m_NeighsAndAngles.push_back(vector<pair<int, double> > ());
	map<int, vector<int>> sequenceOfDegrees;
	map<int, int> startEdges;
	for (set<int>::const_iterator it = allAffectedEdges.begin(); it != allAffectedEdges.end(); ++it)
	{
		int i = *it;
		const CEdge& edge = Edge(i);
		int leftV = edge.indexOfLeftVert;
		if (allVertices.find(leftV) == allVertices.end())
			continue;
		sequenceOfDegrees[leftV].push_back(i);
		if (startEdges.find(leftV) == startEdges.end())
			startEdges[leftV] = i;
		else if (!IsStartEdge(startEdges[leftV]))
			startEdges[leftV] = i;
	}
	for (set<int>::const_iterator it = allVertices.begin(); it != allVertices.end(); ++it)
	{
		int v = *it;
		m_NeighsAndAngles[v].push_back(make_pair(startEdges[v], 0));
		for (int i = 1; i < sequenceOfDegrees[v].size(); ++i)
		{
			m_NeighsAndAngles[v].push_back(make_pair(Edge(m_NeighsAndAngles[v][i-1].first).indexOfLeftEdge, 0));
		}
	}

	//angles
	assert(m_FlagsForCheckingConvexVerts.size() == newV);
	m_FlagsForCheckingConvexVerts.push_back(make_pair(false, false));
...
void CRichModel::SetModel(std::vector<CPoint3D>& vertexs, std::vector<CBaseModel::CFace>&faces)
{
	m_Verts = vertexs;
	m_Faces = faces;
...
int CRichModel::SplitEdge(const EdgePoint& ep)
{
	...
	if (e8 != -1)
		newEdges.insert(e8);
	RenewNeighboursAfterSplit(vertID, ep.index, newEdges);
