...
static inline ICPMatrix lssolvesvd(ICPMatrix &A, ICPMatrix &b)
{
  int i;
  ICPMatrix U, S, V;

  A.svd(U, S, V);
  ICPMatrix Uc=U.getMat(0, 0, A.m-1, A.n-1); // compact U
  ICPMatrix Uctb=~Uc*b;
  for(i=0; i<Uctb.m; ++i)
...
double IcpPointToPlane::fitStep (double *T,const int32_t T_num,ICPMatrix &R,ICPMatrix &t,const std::vector<int32_t> &active) {

  int i;
  int nact = (int)active.size();

  // init matrix for point correspondences
  ICPMatrix p_m(nact,dim); // model
  ICPMatrix p_t(nact,dim); // template
...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,A,b,r00,r01,r10,r11,t0,t1) // schedule (dynamic,2)
    ...
    R = R_*R;
    t = R_*t+t_;
    return max((R_-ICPMatrix::eye(2)).l2norm(),t_.l2norm());
...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,A,b,r00,r01,r02,r10,r11,r12,r20,r21,r22,t0,t1,t2) // schedule (dynamic,2)
    ...
    R = R_*R;
    t = R_*t+t_;
    return max((R_-ICPMatrix::eye(3)).l2norm(),t_.l2norm());
  }
...
std::vector<int32_t> IcpPointToPlane::getInliers (double *T,const int32_t T_num,const ICPMatrix &R,const ICPMatrix &t,const double indist) {
...
void IcpPointToPlane::computeNormal (const kdtree::KDTreeResultVector &neighbors,double *M_normal,const double flatness) {
  ...
  if (dim==2) {
    ...
    ICPMatrix Q = P - ICPMatrix::ones(neighbors.size(),1)*mu;
