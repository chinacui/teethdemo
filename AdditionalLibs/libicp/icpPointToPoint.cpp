...
double IcpPointToPoint::fitStep (double *T,const int32_t T_num,ICPMatrix &R,ICPMatrix &t,const std::vector<int32_t> &active) {
  ...
  // init mean
  ICPMatrix mu_m(1,dim);
  ICPMatrix mu_t(1,dim);
...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,r00,r01,r02,r10,r11,r12,r20,r21,r22,t0,t1,t2) reduction(+:mum0,mum1,mum2, mut0,mut1,mut2) // schedule (dynamic,2)
  ...
  ICPMatrix q_m = p_m - ICPMatrix::ones(active.size(),1)*mu_m;
  ICPMatrix q_t = p_t - ICPMatrix::ones(active.size(),1)*mu_t;
  ...
  if (dim==2) return max((R_-ICPMatrix::eye(2)).l2norm(),t_.l2norm());
  else        return max((R_-ICPMatrix::eye(3)).l2norm(),t_.l2norm());
}

std::vector<int32_t> IcpPointToPoint::getInliers (double *T,const int32_t T_num,const ICPMatrix &R,const ICPMatrix &t,const double indist) {
