...
class ADDITIONALLIBS_CLASS CBaseModel
{
public:	
...
public:
	...
	void SavePamametrizationObjFile(const map<int, pair<double, double>>& uvs, const string& filename) const;
	
	static vector<double> GetScalarField(const char* filename);
