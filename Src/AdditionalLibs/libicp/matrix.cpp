...
ICPMatrix::ICPMatrix (const int32_t m_,const int32_t n_) {
  allocateMemory(m_,n_);
}

ICPMatrix::ICPMatrix (const int32_t m_,const int32_t n_,const FLOAT* val_) {
  allocateMemory(m_,n_);
  int32_t k=0;
...
ICPMatrix::ICPMatrix (const ICPMatrix &M) {
...
ICPMatrix& ICPMatrix::operator= (const ICPMatrix &M) {
  if (this!=&M) {
    if (M.m!=m || M.n!=n) {
      releaseMemory();
      allocateMemory(M.m,M.n);
...
void ICPMatrix::getData(FLOAT* val_,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  int32_t k=0;
...
ICPMatrix ICPMatrix::getMat(int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
...
void ICPMatrix::setMat(const ICPMatrix &M,const int32_t i1,const int32_t j1) {
...
void ICPMatrix::setVal(FLOAT s,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i2<i1 || j2<j1) {
...
void ICPMatrix::setDiag(FLOAT s,int32_t i1,int32_t i2) {
...
ICPMatrix ICPMatrix::extractCols (vector<int> idx) {
  ICPMatrix M(m,idx.size());
  for (int32_t j=0; j<M.n; j++)
    if (idx[j]<n)
...
ICPMatrix ICPMatrix::eye (const int32_t m) {
  ICPMatrix M(m,m);
  for (int32_t i=0; i<m; i++)
    M.val[i][i] = 1;
  return M;
...
ICPMatrix ICPMatrix::ones (const int32_t m,const int32_t n) {
  ICPMatrix M(m,n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      M.val[i][j] = 1;
...
ICPMatrix ICPMatrix::diag (const ICPMatrix &M) {
  if (M.m>1 && M.n==1) {
    ICPMatrix D(M.m,M.m);
    for (int32_t i=0; i<M.m; i++)
      D.val[i][i] = M.val[i][0];
...
ICPMatrix ICPMatrix::reshape(const ICPMatrix &M,int32_t m_,int32_t n_) {
  if (M.m*M.n != m_*n_) {
...
ICPMatrix ICPMatrix::rotMatX (const FLOAT &angle) {
  FLOAT s = sin(angle);
  FLOAT c = cos(angle);
  ICPMatrix R(3,3);
  R.val[0][0] = +1;
...
ICPMatrix ICPMatrix::rotMatY (const FLOAT &angle) {
  FLOAT s = sin(angle);
  FLOAT c = cos(angle);
  ICPMatrix R(3,3);
  R.val[0][0] = +c;
...
ICPMatrix ICPMatrix::rotMatZ (const FLOAT &angle) {
  FLOAT s = sin(angle);
  FLOAT c = cos(angle);
  ICPMatrix R(3,3);
  R.val[0][0] = +c;
...
ICPMatrix ICPMatrix::operator* (const FLOAT &s) {
  ICPMatrix C(m,n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      C.val[i][j] = val[i][j]*s;
...
ICPMatrix ICPMatrix::cross (const ICPMatrix &a, const ICPMatrix &b) {
...
ICPMatrix ICPMatrix::inv (const ICPMatrix &M) {
  if (M.m!=M.n) {
...
bool ICPMatrix::solve (const ICPMatrix &M, FLOAT eps) {
...
bool ICPMatrix::lu(int32_t *idx, FLOAT &d, FLOAT eps) {
...
void ICPMatrix::svd(ICPMatrix &U2,ICPMatrix &W,ICPMatrix &V) {

  ICPMatrix U = ICPMatrix(*this);
  U2 = ICPMatrix(m,m);
  V  = ICPMatrix(n,n);
...
ostream& operator<< (ostream& out,const ICPMatrix& M) {
  if (M.m==0 || M.n==0) {
    out << "[empty matrix]";
  } else {
    char buffer[1024];
...
void ICPMatrix::allocateMemory (const int32_t m_,const int32_t n_) {
  m = abs(m_); n = abs(n_);
  if (m==0 || n==0) {
    val = 0;
    return;
...
FLOAT ICPMatrix::pythag(FLOAT a,FLOAT b) {
  FLOAT absa,absb;
  absa = fabs(a);
  absb = fabs(b);
  if (absa > absb)
